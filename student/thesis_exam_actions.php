...
} catch (Throwable $e) { fail('Database connection error', 500); }
...
} catch (Throwable $e) { fail('Database query error', 500); }

// 5) Πρέπει να είναι EXAM
if (($T['th_status'] ?? '') !== 'EXAM') fail('Η πτυχιακή δεν είναι σε κατάσταση Υπό Εξέταση', 409);
  ...
  try {
    ...
    if ($teacherId > 0) {
      if ((int)$T['supervisor'] === $teacherId) {
        ...
        $qc = $pdo->prepare("SELECT 1 FROM committee WHERE thesisID=? AND (member1=? OR member2=?) LIMIT 1");
        $qc->execute([$reqThesisID, $teacherId, $teacherId]);
        if ($qc->fetchColumn()) $canAccess = true;
      }
    }
  } catch (Throwable $e) { fail('Access check error', 500); }
}
if (!$canAccess) fail('Δεν έχεις δικαίωμα πρόσβασης σε αυτή την πτυχιακή', 403);

// 7) Ensure row στο thesis_exam_meta
try {
  ...
  $pdo->commit();
} catch (Throwable $e) { $pdo->rollBack(); fail('Meta preparation error', 500); }
...
try {
  ...
  if ($action === 'save_draft') {
    ...
    if ($linksRaw !== '') {
      ...
      $valid = [];
      foreach ($linksArr as $L) if (filter_var($L, FILTER_VALIDATE_URL)) $valid[] = $L;
      if ($valid) {
        $linksJson = json_encode(array_values($valid), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if ($linksJson === false) fail('Σφάλμα επεξεργασίας συνδέσμων', 400);
      }
    }

    $fileUploaded = false; $filePath = null; $fileSize = 0;
    if (!empty($_FILES['draft_file']) && $_FILES['draft_file']['error'] === UPLOAD_ERR_OK) {
      ...
      if ($fileSize === 0) fail('Το αρχείο είναι κενό', 400);

      $uploadDir = __DIR__ . '/uploads/exam_drafts/';
      if (!is_dir($uploadDir) && !@mkdir($uploadDir, 0777, true)) fail('Αδυναμία δημιουργίας φακέλου uploads', 500);
      $ext = strtolower(pathinfo($origName, PATHINFO_EXTENSION));
      $fileName = uniqid('draft_'.$reqThesisID.'_', true).'.'.$ext;
      $fullPath = $uploadDir.$fileName;
      $relPath  = 'uploads/exam_drafts/'.$fileName;
      if (!@move_uploaded_file($tmpName, $fullPath)) fail('Αποτυχία αποθήκευσης αρχείου', 500);

      $filePath = $relPath; $fileUploaded = true;
    }

    if ($fileUploaded && $filePath !== null) {
      // delete previous
      $oldQ = $pdo->prepare("SELECT draft_file FROM thesis_exam_meta WHERE thesisID=?");
      $oldQ->execute([$reqThesisID]);
      $old = $oldQ->fetchColumn();
      if ($old && file_exists(__DIR__ . '/' . $old)) @unlink(__DIR__ . '/' . $old);

      $stmt = $pdo->prepare("UPDATE thesis_exam_meta SET draft_file=?, external_links=?, updated_at=NOW() WHERE thesisID=?");
      $stmt->execute([$filePath, $linksJson, $reqThesisID]);
    } else {
      $stmt = $pdo->prepare("UPDATE thesis_exam_meta SET external_links=?, updated_at=NOW() WHERE thesisID=?");
      $stmt->execute([$linksJson, $reqThesisID]);
    }

    $msg = 'Αποθηκεύτηκαν τα στοιχεία draft';
    if ($fileUploaded) $msg .= ' και το αρχείο (' . round($fileSize / 1024, 1) . ' KB)';
    ok(['message'=>$msg,'file_uploaded'=>$fileUploaded]);
  }
  ...
  if ($action === 'save_schedule') {
    ...
    $q = $pdo->prepare("UPDATE thesis_exam_meta SET exam_datetime=?, exam_room=?, exam_meeting_url=?, updated_at=NOW() WHERE thesisID=?");
  ...
  if ($action === 'save_after') {
    ...
    $q = $pdo->prepare("UPDATE thesis_exam_meta SET report_url=?, repository_url=?, updated_at=NOW() WHERE thesisID=?");
